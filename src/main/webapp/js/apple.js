...
EC$(function(){
    var obj = EC$(':input[name="comment_password"]');
    var label = obj.attr('fw-label');
    ...
    obj.attr('fw-label', label);    
    
    // 댓글쓰기
    BOARD_COMMENT.comment_byte('commentWriteForm');
    BOARD_COMMENT.comment_byte('commentReplyWriteForm');
});
...
var BOARD_COMMENT = {
    ...
    //댓글에 댓글 저장중 확인 플래그
    is_comment_reply_submitting : false,
    //댓글 수정 확인 플래그
    is_comment_update_submitting : false,
    
    //댓글 저장 요청 URL
    comment_insert_url : false,
    //댓글에 댓글 저장 요청 URL
    comment_reply_insert_url : false,

    // ECHOSTING-247230 댓글길이 제한이 무제한이더라도 최고 길이는 12000byte로 제한
    comment_maximun_size : 12000,
     ...
     * @returns {Boolean}
     */
    comment_insert : function(action_url)
    {
        this.comment_insert_url = action_url;

        var result = FwValidator.inspection('commentWriteForm');
        if (result.passed) {

            if (BOARD_COMMENT.is_comment_submitting === false) {
                BOARD_COMMENT.is_comment_submitting = true;
                
                //SSL 처리
                var aEleId = [EC$("#commentWriteForm #comment_name"),
                              EC$("#commentWriteForm #comment_password")];
                
                AuthSSLManager.weave({
                    'auth_mode' : 'encrypt',
                    'aEleId' : aEleId,
                    'auth_callbackName' : 'BOARD_COMMENT.comment_insert_ajax'
                });
            }
        } else {
            BOARD_COMMENT.is_comment_submitting = false;
        }
    ...
    comment_insert_ajax : function(output)
    {
        ...
        var form = EC$('#commentWriteForm');
        
        //암호화된 필드 내 값 제거
        EC$("#commentWriteForm #comment_name").val('');
        EC$("#commentWriteForm #comment_password").val('');
        ...
        EC$.post(BOARD_COMMENT.comment_insert_url, EC$.param(formData), function(req) {
            if (!req.failed) {
                self.location.reload();
            } else {
                BOARD_COMMENT.is_comment_submitting = false;
                form.children("input[name='"+AuthSSL.sEncryptId+"']").remove();
     ...
     * @returns {Boolean}
     */
    comment_reply_insert : function(action_url)
    {
        this.comment_reply_insert_url = action_url;
        
        var result = FwValidator.inspection('commentReplyWriteForm');
        if (result.passed) {

            if (BOARD_COMMENT.is_comment_reply_submitting === false) {
                BOARD_COMMENT.is_comment_reply_submitting = true;

                //SSL 처리
                var aEleId = [EC$("#commentReplyWriteForm #comment_name"),
                              EC$("#commentReplyWriteForm #comment_password")];
                
                AuthSSLManager.weave({
                    'auth_mode' : 'encrypt',
                    'aEleId' : aEleId,
                    'auth_callbackName' : 'BOARD_COMMENT.comment_reply_insert_ajax'
                });
            }
        } else {
            BOARD_COMMENT.is_comment_reply_submitting = false;
        }
    ...
    comment_reply_insert_ajax : function(output)
    {
        ...
        var form = EC$('#commentReplyWriteForm');
        
        //암호화된 필드 내 값 제거
        var sTempCommentName = '';
        sTempCommentName = EC$("#commentReplyWriteForm #comment_name").val();

        EC$("#commentReplyWriteForm #comment_name").val('');
        EC$("#commentReplyWriteForm #comment_password").val('');
        ...
        form.append(oInput);
        var formData = form.serializeArray() || [];
        // 비밀글 선택되어 있고, disabled된 경우 ECHOSTING-115448에 의해 추가됨
        if (EC$('[name=secure]:disabled', form).val() == 'T') {
            formData.push({name: 'secure', value: 'T'});
        }        
        EC$.post(this.comment_reply_insert_url, EC$.param(formData), function(req) {
            if (!req.failed) {
                self.location.reload();
            } else {
                BOARD_COMMENT.is_comment_reply_submitting = false;
                form.children("input[name='"+AuthSSL.sEncryptId+"']").remove();
                EC$("#commentReplyWriteForm #comment_name").val(sTempCommentName);
                alert(decodeURIComponent(req.msg));
                return false;
            }
        },'json');  
    ...
    comment_delete_submit : function(obj)
    {
        EC$(obj).parents('form').submit();
    },
     ...
     * @returns array
     */
    commentGet : function(iNo, iCommentNo)
    {
        var form = EC$("#commentForm").get(0);
        var iBoardNo = form.board_no.value;
        var action_url = '/exec/front/board/CommentGet/' + iBoardNo + '/';
        ...
        //ECHOSTING-45947 댓글 수정시 비번 입력후 엔터키 방지
        if (form.comment_password !== '') {
            EC$("#comment_password").on('keypress',function(e) {
                 if (e.keyCode==13) return false;
            });
        }

        EC$.get(action_url, aData, function(req) {
            if (req.failed == false) {
                var rData = req.data;
                BOARD_COMMENT.get_comment_callback('commentForm', rData);
            }
        }, 'json');
     ...
     * @param e
     */
    comment_reply_write : function(iCommentNo, e)
    {
        ...
        if ($form.css('display') == 'none') {
            ...
            try {
                var sCommentName = $form.get(0).comment_name.value;

            $form.get(0).reset();
            $form.get(0).comment_no.value = iCommentNo;
                $form.get(0).comment_name.value = sCommentName;
            } catch(e) {}

            var $p = EC$(e).parent();
            if ( $p.parent().find('#commentReplyWriteForm').length < 1 ) {
                $p.after($form);
            }
        } else {
            $form.hide();
     ...
     * @param e
     */
    comment_reply_write_new : function (iCommentNo, e)
    {
        EC$('#commentForm').css('display', 'none');
        ...
        if ($form.css('display') == 'none') {
            ...
            var $p = EC$(e).parent().parent();
            if ( EC$(e).parent().find('#commentReplyWriteForm').length < 1 ) {
                $p.after($form);
            }
        } else {
            $form.hide();
     ...
     * @param e
     */
    comment_update : function(iNo, iCommentNo, e)
    {
        var $p = EC$(e).parent();
        var $form = EC$("#commentForm").css('display', 'none');
        $p.after($form);
    ...
    /**
     * 댓글 수정 모바일 뉴입력폼 출력 (신규스킨)
     * @param bbs_no
     * @param comment_no
     * @param e
     */
    comment_update_new : function(iNo, iCommentNo, e)
    {
        var $p = EC$(e).parent().parent();
        var sCommentName = EC$("#commentForm").get(0).comment_name.value;

        this.comment_cancel_ok_new('commentForm');
        this.hide_secret_comment_form('commentSecretForm');

        $p.after(EC$("#commentForm"));

        // 비밀댓글인 경우 비밀번호 확인 (ECHOSTING-250560)
        if (EC$('#comment_secret_link'+iCommentNo).length > 0) {
            // 비밀번호 확인 이후 실행되어야 할 콜백 등록
            this.set_after_filter(BOARD_COMMENT.get_comment_callback);
            this.show_secret_comment_form('commentSecretForm', iCommentNo, true);
        ...
        this.commentGet(iNo, iCommentNo);
        EC$("#commentForm").get(0).comment_name.value = sCommentName;
    ...
    /**
     * 댓글 삭제 폼 출력
     * @param link 삭제 페이지 URL
     */
    comment_delete : function(link)
    {
       this.setLocationPage(link);
    },
     ...
     * @param link 삭제 페이지 URL
     */
    comment_reply_delete : function(iCommentNo, link)
    {
        var form = EC$("#commentForm").get(0);
        var iBoardNo = form.board_no.value;
        var action_url = '/exec/front/board/CommentGet/' + iBoardNo + '/';
        ...
        EC$.get(action_url, aData, function(req) {
            if (req === null) {
                //ECHOSTING-250560 비회원으로 비밀글로 댓글작성후 삭제버튼 클릭시 반응없음
                BOARD_COMMENT.setLocationPage(link);
            }
            else if (req.data.reply_count > 0) {
                sAddParam = '&commentReply=T';
                BOARD_COMMENT.setLocationPage(link + '&commentReply=T');
            } else {
                BOARD_COMMENT.setLocationPage(link);
            }
        }, 'json');
     ...
     * @returns {Boolean}
     */
    comment_delete_opdiary : function (iBoardNo, iNo, iCommentNo, sMemberId)
    {
        if (confirm(__('삭제하시겠습니까?')) == false) return false;
        ...
        EC$.post('/exec/front/Board/CommentDel/'+iBoardNo, aData, function(req) {
            if (!req.failed) {
                self.location.reload();
            } else {
                alert(req.msg);
                if (req.action) top.location.href(req.action);
                return false;
            }
        },'json');
    },
     ...
     * @param fname 폼이름
     */
    comment_update_ok : function(fname)
    {
        var result = FwValidator.inspection(fname);
        if (result.passed) {
            if (BOARD_COMMENT.is_comment_update_submitting === false) {
                BOARD_COMMENT.is_comment_update_submitting = true;

                //SSL 처리
                var aEleId = [EC$("#commentForm #comment_password")];

                if (EC$('#commentForm #comment_name').length > 0) {
                    aEleId.push(EC$('#commentForm #comment_name'));
                }
                
                AuthSSLManager.weave({
                    'auth_mode' : 'encrypt',
                    'aEleId' : aEleId,
                    'auth_callbackName' : 'BOARD_COMMENT.comment_update_ajax'
                });
            }
        } else {
            BOARD_COMMENT.is_comment_update_submitting = false;
        }
    },
     ...
     * @param callbackParam
     */
    comment_update_ajax : function(output)
    {
        //SSL 실패확인
        var output = decodeURIComponent(output);
        if (AuthSSLManager.isError(output) == true) {
        ...
        var form = EC$('#commentForm');
        
        //암호화된 필드 내 값 제거
        EC$("#commentForm #comment_password").val('');
        ...
        EC$.post(form.attr("action"), EC$.param(formData), function(req) {
            if (!req.failed) {
                self.location.reload();
            } else {
                BOARD_COMMENT.is_comment_update_submitting = false;
     ...
     * @param fname
     */
    comment_cancel_ok : function(fname)
    {
        var $form = EC$('#'+fname);
     ...
     * @param fname
     */
    comment_cancel_ok_new : function(fname)
    {
        var $form = EC$('#'+fname);
        $form.css('display', 'none');
        
        $form.prev().css('display', '');
    },

    /**
     * 키보드 입력시 체크하기
     */
    comment_byte : function(sFormName)
    {
        var targetByte = EC$('#'+sFormName).find('#comment_byte');
        var comment_size = EC_UTIL.trim(targetByte.attr('title'));
        var content = 'comment';
        // ECHOSTING-247259 댓글입력 사이즈가 무제한일 경우에는 12000byte 제한
        if (comment_size == '') comment_size = this.comment_maximun_size;
        if (sFormName == 'commentForm') {
            content = 'comment_modify';
            BOARD_COMMENT.limitWord(EC$('#'+content).get(0), comment_size, sFormName);
        }

        if (comment_size > 0) {
            EC$('#'+sFormName).find('#'+content).on('keyup', function() {
                BOARD_COMMENT.limitWord(this, comment_size, sFormName);
            });
        }
    },
    ...
    limitWord : function(txt, limit, sFormName)
    {
        ...
        if ( strLen > limit ) {
            alert(sprintf(__('메시지는 최대 %s Byte 이하로 입력해주세요.'), limit));
            
            var aChar = [];
            var iMax = txt.value.length;
            for ( var i = 0; i < iMax; i++) {
                var char = txt.value.substr(i, 1);
                var tmpLen = this.stringByteSize(aChar.join('')+char);
            ...
            txt.value = aChar.join('');
            strLen = this.stringByteSize(txt.value);
        }
    ...
    /**
     * 비밀댓글 확인 폼 열기
     * @param string sFormName 폼이름
     * @param int iCommentNo 코멘트 번호
     * @param boolean bResetAfterFilter 필터 리셋 여부
     */
    show_secret_comment_form : function(sFormName, iCommentNo, bAfterFilter)
    {
        var $form = EC$('#' + sFormName);
        var bAfterFilter = bAfterFilter || false;

        // 댓글 확인 이후 필터 리셋
        if (bAfterFilter == false) {
            this.reset_after_filter();
        }

        if ($form.length > 0) {
            // 폼 리셋
            $form[0].reset();
            EC$('#comment_no', $form).val(iCommentNo);
            $form.removeClass('displaynone').css('display', 'block');
            EC$('#comment_secret_link' + iCommentNo).parent().after($form);
            if ($form.secure_password !== '') {
                EC$("#secure_password", $form).on('keypress',function(e) {
                     if (e.keyCode==13) return false;
                });
            }
        }
    ...
    /**
     * 비밀댓글 확인 폼 닫기 
     * @param string sFormName 폼 이름
     */
    hide_secret_comment_form : function(sFormName) 
    {
        var $form = EC$('#' + sFormName);
    
        if ($form.length > 0 ) {
            $form.removeClass('displaynone').css('display', 'none');
        }
    },
     ...
     * @param string sFormName 폼 이름
     */
    show_secret_comment : function(sFormName)
    {
        var $form = EC$('#' + sFormName);
        var oValidateResult = FwValidator.inspection(sFormName);
        if (oValidateResult.passed) {
            var aId = [];
            if (EC$('#secure_password', $form).val()) {
                aId.push(sFormName+'::secure_password');
            }
            ...
            AuthSSL.encrypt(AuthSSL.aEleId, 'BOARD_COMMENT.show_secret_comment_ajax');
        }
    },
    ...
    show_secret_comment_ajax : function(sOutput)
    {
        ...
        EC$('#'+AuthSSL.sEncryptId, $form).val(sOutput);
        EC$.post($form.attr('action'), $form.serialize(), function(req){
            if (req.failed) {
                alert(req.msg);
                EC$("input[name='"+ AuthSSL.sEncryptId +"']", $form).remove();
                EC$('[id=secure_password]', $form).focus();
                return;
            }
            var aData = req.data;
            var sMode = $form.data('mode') || '';
        
            EC$('#comment_contents' + aData.comment_no).html(aData.comment);
            BOARD_COMMENT.hide_secret_comment_form(sFormName);
            BOARD_COMMENT.apply_after_filter(['commentForm', aData]);
        }, 'json');
    },
     ...
     * 댓글 확인 후 처리
     * @param string sFormName 폼 이름
     * @param string aData 댓글정보
     */
    get_comment_callback : function(sFormName, aData)
    {
        var sFormName = sFormName || 'commentForm';
        var $form = EC$('#' + sFormName);

        // ECHOSTING-314621 댓글 수정시 작성자명 폼에 댓글 작성자명을 채워주도록 수정
        if (EC$('#comment_name', $form).length > 0 && 'comment_name' in aData === true) {
            EC$('#comment_name', $form).val(aData['comment_name']);
        }

        EC$('#comment_modify', $form).val(aData['comment']);
        EC$('#member_id', $form).val(aData['member_id']);
        EC$('#comment_no', $form).val(aData['comment_no']);
        EC$('#comment_password', $form).val('');
        if (EC$('[name=secure]', $form).length > 0 && aData['secret_flag'] == 'T') {
            EC$('[name=secure]', $form).prop('checked', true);
        }
        BOARD_COMMENT.comment_byte(sFormName);
        $form.css('display', 'block');
        EC$('#commentReplyWriteForm').css('display', 'none');
    },
     ...
     */
    set_after_filter : function(callback)
    {
     ...
     * @param function callback
     */    
    reset_after_filter : function()
    {
        this.after_filter = null;
    },
     ...
     * @param function callback
     */
    apply_after_filter : function(aParam)
    {
        var aParam = aParam || [];
        
        if (typeof this.after_filter == 'function') {
            this.after_filter.apply(this, aParam);
        }
        this.reset_after_filter();
    },
     ...
     */
    setCmtData: function () {
        var $commentNameInput = EC$('input[name="comment_name"]', '.xans-board-commentwrite, .xans-board-commentreply');
        // 게시글 읽기 화면이 아닐 때 세팅하지 않음
        if ($commentNameInput.length < 1 || typeof EC_FRONT_JS_CONFIG_BOARD === 'undefined') {
            return;
        }

        var oMemberData = EC_FRONT_JS_CONFIG_BOARD.iBoardNo == 11 ? parent.CAPP_ASYNC_METHODS.member.getData() : CAPP_ASYNC_METHODS.member.getData();
        var sMemberId = oMemberData.member_id;
        var sName = oMemberData.name;
        var sNickName = oMemberData.nick_name;

        var sCommentWriterType = EC_FRONT_JS_CONFIG_BOARD.sCommentWriterType;
        var bCommentUseId = EC_FRONT_JS_CONFIG_BOARD.sCommentUseId === 'T';

        // 운영일지
        if (EC_FRONT_JS_CONFIG_BOARD.iBoardNo == 11) {
            sCommentWriterType = 'N';
            if (bCommentUseId === true) sName = sMemberId;
        }

        var sWriteName = '';

        // 게시판 설정에 따라 설정
        if (sCommentWriterType === 'N') {
            sWriteName = sName;
        } else if (sCommentWriterType === 'U') {
            sWriteName = sNickName === '' ? sName : sNickName;
        } else if (sCommentWriterType === 'I') {
            sWriteName = sNickName === '' ? sMemberId : sNickName;
        }

        // 대표관리자는 이름으로 설정
        var sMallId = EC_GLOBAL_INFO.getBaseDomain().split('.')[0];
        if (sMemberId == sMallId) {
            sWriteName = oMemberData.board_write_name;
        }

        if (sWriteName === '') {
            sWriteName = sMemberId;
        }

        // 작성자명 세팅
        $commentNameInput.val(sWriteName);

        // 댓글 초기비밀번호 세팅
        if (typeof sMemberId !== 'undefined' && sMemberId !== null && sMemberId.length > 0) {
            var sPw = decodeURIComponent(sMemberId).split('').reverse().join('');
            EC$('input[name="comment_password"]', '.xans-board-commentwrite').val(sPw);
        }
    },
